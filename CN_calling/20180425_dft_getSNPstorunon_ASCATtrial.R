...
write.table(x = data.frame(chr = baflogr_run1$chr, pos = baflogr_run1$cumpos, DFT2 = baflogr_run1$logr), file = "20180425_mixtest_tumour_LogR.txt", sep = "\t", row.names = T, quote = F)
write.table(x = data.frame(chr = baflogr_run1$chr, pos = baflogr_run1$cumpos, Fibro = 0), file = "20180425_mixtest_host_LogR.txt", sep = "\t", row.names = T, quote = F)

write.table(x = data.frame(chr = baflogr_run1$chr, pos = baflogr_run1$cumpos, DFT2 = abs(baflogr_run1$baf - sample(x = c(0,1), size = nrow(baflogr_run1), replace = T))), file = "20180425_mixtest_tumour_BAF.txt", sep = "\t", row.names = T, quote = F)
write.table(x = data.frame(chr = baflogr_run1$chr, pos = baflogr_run1$cumpos, Fibro = abs(baflogr_run1$h_alt/(baflogr_run1$h_ref+baflogr_run1$h_alt) - sample(x = c(0,1), size = nrow(baflogr_run1), replace = T))), file = "20180425_mixtest_host_BAF.txt", sep = "\t", row.names = T, quote = F)


### running ascat do it twice for Fibro and DFT2
library(ASCAT)
ascat.bc <- ascat.loadData("20180425_mixtest_tumour_LogR.txt", "20180425_mixtest_tumour_BAF.txt","20180425_mixtest_host_LogR.txt","20180425_mixtest_host_BAF.txt")
...
write.table(x = ascat.output$segments, file = "Fibro.ASCATprofile.segments.txt", sep = "\t", quote = F, row.names = F)
...
write.table(x = data.frame(chr = baflogr_run2$chr, pos = baflogr_run2$cumpos, DFT2 = baflogr_run2$logr), file = "20180425_mixtest_tumour_LogR.txt", sep = "\t", row.names = T, quote = F)
write.table(x = data.frame(chr = baflogr_run2$chr, pos = baflogr_run2$cumpos, Fibro = 0), file = "20180425_mixtest_host_LogR.txt", sep = "\t", row.names = T, quote = F)

write.table(x = data.frame(chr = baflogr_run2$chr, pos = baflogr_run2$cumpos, DFT2 = abs(baflogr_run2$baf - sample(x = c(0,1), size = nrow(baflogr_run2), replace = T))), file = "20180425_mixtest_tumour_BAF.txt", sep = "\t", row.names = T, quote = F)
write.table(x = data.frame(chr = baflogr_run2$chr, pos = baflogr_run2$cumpos, Fibro = abs(baflogr_run2$normalbaf - sample(x = c(0,1), size = nrow(baflogr_run2), replace = T))), file = "20180425_mixtest_host_BAF.txt", sep = "\t", row.names = T, quote = F)
